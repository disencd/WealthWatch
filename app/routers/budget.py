...
def _sub_dict(s: SubCategory, cat: Category | None = None) -> dict:
    d = {
        "id": s.id,
...
async def list_subcategories(
    ...
    q = (
        ...
        .where(SubCategory.family_id == current_user.family_id)
    ...
    sub = (
        await db.execute(
            select(SubCategory).where(
                SubCategory.id == req.sub_category_id, SubCategory.family_id == current_user.family_id
            )
        )
    ).scalar_one_or_none()
    if not sub:
...
async def monthly_summary(
    ...
    sub_totals = (
        await db.execute(
            select(
                BudgetExpense.category_id,
                BudgetExpense.sub_category_id,
                SubCategory.name,
    ...
    return {
        "year": year,
        "month": month,
        "total_spent": float(total_row or 0),
        "by_category": [{"category_id": r[0], "category_name": r[1], "total_amount": float(r[2])} for r in cat_totals],
        "by_subcategory": [
            {"category_id": r[0], "sub_category_id": r[1], "sub_category_name": r[2], "total_amount": float(r[3])}
            for r in sub_totals
        ],
...
async def import_categories_csv(
    ...
    async def upsert_sub(category_id: int, name: str):
        ...
        existing = (
            await db.execute(
                select(SubCategory).where(
                    SubCategory.family_id == fid, SubCategory.category_id == category_id, SubCategory.name == name
                )
            )
        ).scalar_one_or_none()
        if existing:
...
async def import_monthly_csv(
    ...
    await db.commit()
    return {
        "created_budget_expenses": total_created,
        "skipped": total_skipped,
        "created_categories": total_cats,
        "created_sub_categories": total_subs,
    }
