...
def _holding_dict(h: InvestmentHolding) -> dict:
    return {
        ...
        "symbol": h.symbol,
        "name": h.name,
        "investment_type": h.investment_type.value,
        "quantity": h.quantity,
    ...
    rows = (
        (
            await db.execute(
                ...
                .where(InvestmentHolding.family_id == current_user.family_id)
...
async def create_holding(
    ...
    h = InvestmentHolding(
        ...
        symbol=req.symbol,
        name=req.name,
        investment_type=InvestmentType(req.investment_type),
...
async def get_portfolio_summary(
    ...
    rows = (
        (await db.execute(select(InvestmentHolding).where(InvestmentHolding.family_id == current_user.family_id)))
        .scalars()
        .all()
    )
    ...
    h = (
        await db.execute(
            select(InvestmentHolding).where(
                InvestmentHolding.id == id, InvestmentHolding.family_id == current_user.family_id
            )
        )
    ).scalar_one_or_none()
    if not h:
