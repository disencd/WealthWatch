    ...
    return [
        {
            ...
            "members": [
                {"id": m.id, "first_name": m.first_name, "last_name": m.last_name, "email": m.email} for m in g.members
            ],
        }
        for g in rows
    ]
...
async def get_group(
    ...
    g = (
        await db.execute(select(Group).options(selectinload(Group.members)).where(Group.id == id))
    ).scalar_one_or_none()
    if not g:
...
async def add_member(
    ...
    if not g:
        raise HTTPException(404, "Group not found")
    existing = (
        await db.execute(select(GroupMember).where(GroupMember.group_id == id, GroupMember.user_id == req.user_id))
    ).scalar_one_or_none()
    if existing:
...
async def remove_member(
    ...
    gm = (
        await db.execute(select(GroupMember).where(GroupMember.group_id == id, GroupMember.user_id == member_id))
    ).scalar_one_or_none()
    if not gm:
